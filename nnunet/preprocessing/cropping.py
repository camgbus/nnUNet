...
def get_case_identifier(case):
    case_identifier = os.path.split(case[0])[1].split(".nii.gz")[0][:-5]
    return case_identifier


def get_case_identifier_from_npz(case):
    case_identifier = os.path.split(case)[1][:-4]
    return case_identifier
...
def get_patient_identifiers_from_cropped_files(folder):
    return [os.path.split(i)[1][:-4] for i in subfiles(folder, join=True, suffix=".npz")]
...
class ImageCropper(object):
    ...
    def get_patient_identifiers_from_cropped_files(self):
        return [os.path.split(i)[1][:-4] for i in self.get_list_of_cropped_files()]
